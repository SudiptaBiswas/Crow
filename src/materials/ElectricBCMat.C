...
registerMooseObject("CrowApp", ElectricBCMat);

template <> InputParameters validParams<ElectricBCMat>() {
  ...
  params.addParam<MooseEnum>("bc_type", bc_type,
                             "Type of boundary condition. ");
  MultiMooseEnum boundary_side("Left Right Top Bottom Front Back", "Top");
  params.addRequiredParam<MultiMooseEnum>("boundary_side", boundary_side,
                                          "Specifying boundary side to apply "
                                          "the boundary condition as the "
                                          "materials property.");
  return params;
}

ElectricBCMat::ElectricBCMat(const InputParameters &parameters)
    : DerivativeMaterialInterface<Material>(parameters), _c_var(coupled("c")),
      _c(coupledValue("c")), _elec(coupledValue("elec")),
      _grad_c(coupledGradient("c")), _op_num(coupledComponents("v")),
      _vals(_op_num), _left_func(getFunction("left_function")),
...
void ElectricBCMat::computeQpProperties() {
  Real tol = 0.01;
  RealGradient ns(0);
  ...
  case 0:
    ...
    // std::cout << "##qp = " << _qp << ", delecbc = " << _delec_bc[_qp] << ",
    // ns1 = " << fabs(ns(1)) << std::endl;
  ...
  case 1: // Neumann
    for (unsigned int num = 0; num < _boundary_num; ++num) {
      if (_boundary_side[num] == "Top")
        // if (_q_point[_qp](0) > 10.0 && _q_point[_qp](0) < 14.0 &&
        // _q_point[_qp](1) > 5.0 && _q_point[_qp](1) < 7.0)
