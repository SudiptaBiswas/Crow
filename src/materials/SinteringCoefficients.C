...
InputParameters validParams<SinteringCoefficients>()
{
  ...
  params.addParam<bool>("anisotropic", false, "GB mobility input in m^4/(J*s), that overrides the temperature dependent calculation");
...
void
SinteringCoefficients::computeProperties()
{
  Real energy_scale = _energy_scale; // Decide energy scale based on energy unit
  if (_energy_unit == 0)
    energy_scale *= _JtoeV;

  const Real GB_energy = _GB_energy / energy_scale * _length_scale * _length_scale; // Non-dimensionalized GB energy
