...
registerMooseObject("CrowApp", SinteringMobility);

template <> InputParameters validParams<SinteringMobility>() {
...
SinteringMobility::SinteringMobility(const InputParameters &parameters)
    : Material(parameters), _T(coupledValue("T")), _c(coupledValue("c")),
      _grad_c(coupledGradient("c")), _D(declareProperty<Real>("D")),
      _Dbulk(declareProperty<Real>("Dbulk")),
      _Dsurf(declareProperty<Real>("Dsurf")),
      _Dgb(declareProperty<Real>("Dgb")), _dDdc(declareProperty<Real>("dDdc")),
      _M(declareProperty<Real>("M")), _dMdc(declareProperty<Real>("dMdc")),
...
void SinteringMobility::computeProperties() {
  const Real D0_c =
      _D0 * _time_scale /
      (_length_scale *
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp) {
    ...
    // Real Msurf
    // = 2.0/3.0*Dsurf*mult_surf*omega_kT*_energy_scale[_qp]*_ls/(int_width_c*_length_scale);
    // Real dMsurfdc
    // = 2.0/3.0*Dsurf*dmult_surf*omega_kT*_energy_scale[_qp]*_ls/(int_width_c*_length_scale);
