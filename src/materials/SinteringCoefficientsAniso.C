...
InputParameters validParams<SinteringCoefficientsAniso>()
{
  ...
  params.addParam<Real>("energy_scale", 1.0e-9, "defines the base energy scale of the problem in eV");
  MooseEnum energy_unit("Joule eV", "eV");
  params.addParam<MooseEnum>("energy_unit", energy_unit, "Specify the energy unit for surface and Gb energy." );
...
SinteringCoefficientsAniso::SinteringCoefficientsAniso(const InputParameters & parameters) :
    Material(parameters),
...
void
SinteringCoefficientsAniso::computeProperties()
{
  ...
  GB_energy *= (_JtoeV * _length_scale * _length_scale / energy_scale); // Non-dimensionalized GB energy
  const Real surface_energy = _surface_energy / energy_scale * _length_scale * _length_scale;
  ...
  const Real GBmob0_c = _GBmob0 * _time_scale / ((energy_scale*_JtoeV) * (_length_scale*_length_scale*_length_scale*_length_scale)); // Convert to lengthscale^4/(eV*timescale);
  ...
  for (_qp = 0; _qp < _qrule->n_points(); ++_qp)
  {
    ...
    if (_GBMobility < 0)
      ...
      GBmob = _GBMobility * _time_scale / ((energy_scale*_JtoeV) * (_length_scale*_length_scale*_length_scale*_length_scale));; // GBMobility in m^4/(J*s)

    Real L_iso = 4.0/3.0 * GBmob / int_width_c; // Non-dimensionalized Allen-Cahn Mobility
