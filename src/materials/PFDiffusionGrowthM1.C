#include "PFDiffusionGrowthM1.h"

registerMooseObject("CrowApp", PFDiffusionGrowthM1);

template <> InputParameters validParams<PFDiffusionGrowthM1>() {
  ...
  params.addRequiredCoupledVar("Dvol", "phase field variable");
...
PFDiffusionGrowthM1::PFDiffusionGrowthM1(const InputParameters &parameters)
...
void PFDiffusionGrowthM1::computeQpProperties() {
  Real SumEtaj = 0.0;
  for (unsigned int i = 0; i < _ncrys; ++i)
  ...
  _D[_qp] = Dv * phi + _Dvap * (1.0 - phi) + _Dsurf * c * (1 - c) +
            _Dgb * SumEtaj; // + _Dvap*(1 - phi) ;

  Real dphidc = 30.0 * c * c * (1 - 2 * c + c * c);
  _dDdc[_qp] = Dv * dphidc - _Dvap * dphidc + _Dsurf * (1.0 - 2.0 * c);
