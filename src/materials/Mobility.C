...
registerMooseObject("CrowApp", Mobility);

template <> InputParameters validParams<Mobility>() {
  InputParameters params = validParams<Material>();
  params.addClassDescription("Phase field parameters for polynomial free "
                             "energy for single component systems");
  params.addCoupledVar("T", "Temperature variable in Kelvin");
  params.addRequiredCoupledVar("c", "Concentration");
  params.addRequiredParam<Real>("int_width", "The interfacial width of void "
                                             "surface in the lengthscale of "
                                             "the problem");
  ...
  params.addRequiredParam<Real>("D0",
...
Mobility::Mobility(const InputParameters &parameters)
    : Material(parameters), _c(coupledValue("c")),
      _c_name(getVar("c", 0)->name()), _T(coupledValue("T")),
      ...
      _time_scale(getParam<Real>("time_scale")), _D0(getParam<Real>("D0")),
...
void Mobility::computeQpProperties() {
