...
registerMooseObject("CrowApp", TungstenProp);

template <> InputParameters validParams<TungstenProp>() {
  ...
  params.addParam<Real>("length_scale", 1e-9,
                        "Length scale w.r.t m for unit conversion.");
...
TungstenProp::TungstenProp(const InputParameters &parameters)
    : DerivativeMaterialInterface<Material>(parameters), _T(coupledValue("T")),
...
void TungstenProp::computeQpProperties() {
  _density[_qp] =
      19.3 * 100e3 * _mass_scale /
      (_length_scale * _length_scale *
       _length_scale); // original unit g/cm^3 converted to simulations scale
  _thermal_conductivity[_qp] =
      240.51 - 0.2899 * _T[_qp] + 2.5403e-4 * _T[_qp] * _T[_qp] -
