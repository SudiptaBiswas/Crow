...
template <> InputParameters validParams<PFEigenStrainBaseMaterial>() {
...
PFEigenStrainBaseMaterial::PFEigenStrainBaseMaterial(
    const InputParameters &parameters)
    : DerivativeMaterialInterface<ComputeEigenstrain>(parameters),

      _c(coupledValue("c")), _c_name(getVar("c", 0)->name()),
      ...
      _delastic_strain_dc(declarePropertyDerivative<RankTwoTensor>(
          _base_name + "elastic_strain", _c_name)),
      _d2elastic_strain_dc2(declarePropertyDerivative<RankTwoTensor>(
          _base_name + "elastic_strain", _c_name, _c_name))

// _delasticity_tensor_dc(declarePropertyDerivative<ElasticityTensorR4>(_elasticity_tensor_name,
// _c_name)),
// _d2elasticity_tensor_dc2(declarePropertyDerivative<ElasticityTensorR4>(_elasticity_tensor_name,
// _c_name, _c_name))
{
