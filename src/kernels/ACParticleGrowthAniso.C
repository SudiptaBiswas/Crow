...
InputParameters validParams<ACParticleGrowthAniso>()
{
  ...
  params.addRequiredParam<UserObjectName>("gbenergymap", "Where the map of the energies are held");
  return params;
}
...
Real
ACParticleGrowthAniso::computeDFDOP(PFFunctionType type)
{
  // start sums at zero
  Real SumEtaj = 0.0;
  Real SumEtaij = 0.0;
  ...
  const AnisoGBEnergyUserObject::SigmaIJMap & gb_energy_map = _aniso_GB_energy.getGBEnergies(_current_elem->id());
  if (gb_energy_map.size() >= 2)
  {
    std::set<unsigned int> op_set;
    for (AnisoGBEnergyUserObject::SigmaIJMap::const_iterator it = gb_energy_map.begin(); it != gb_energy_map.end(); ++it)
    {
      ...
      SumEtaSigmaij += sigmaij * (*_vals[op1])[_qp] * (*_vals[op1])[_qp] * (*_vals[op2])[_qp] * (*_vals[op2])[_qp];
    }
    if (op_set.count(_op))
    {
      for (std::set<unsigned int>::const_iterator it = op_set.begin(); it != op_set.end(); ++it)
      {
        if (*it != _op)
        {
          ...
          SumEtaj3 += (*_vals[*it])[_qp] * (*_vals[*it])[_qp] * (*_vals[*it])[_qp];
          SumGradEta += ((*_grad_vals[*it])[_qp] * (*_grad_vals[*it])[_qp]);

          Real sigmaiop;
          // The lower order parameter is first in the pair TODO: check if stuff was found!!!
          if (_op > *it)
      ...
      Dsigma_Deta = 2.0 * _u[_qp] * (SumEtaSigmaj * SumEtaij - SumEtaj * SumEtaSigmaij) / (SumEtaij * SumEtaij);
      D2sigma_Deta2 = 2.0 * (SumEtaSigmaj * SumEtaij - SumEtaj * SumEtaSigmaij) / (SumEtaij * SumEtaij);
  ...
  {
    case Residual:
      return -(7.0 / _int_width) * Dsigma_Deta * _c[_qp] * _c[_qp] * (1.0 - _c[_qp]) * (1.0 - _c[_qp])
             + Dsigma_Deta / _int_width * (_c[_qp] * _c[_qp] + 6.0 * (1.0 - _c[_qp]) * SumEtaj
             - 4.0 * (2.0 - _c[_qp]) * SumEtaj3 + 3.0 * SumEtaj * SumEtaj)
             + 0.75 * Dsigma_Deta * _int_width * SumGradEta;

    case Jacobian:
    return _phi[_j][_qp] * (-7.0 / _int_width) * D2sigma_Deta2 * _c[_qp] * _c[_qp] * (1.0 - _c[_qp]) * (1.0 - _c[_qp])
           + _phi[_j][_qp] * D2sigma_Deta2 / _int_width * (_c[_qp] * _c[_qp] + 6.0 * (1.0 - _c[_qp]) * SumEtaj
           - 4.0 * (2.0 - _c[_qp]) * SumEtaj3 + 3.0 * SumEtaj * SumEtaj)
...
Real
ACParticleGrowthAniso::computeQpOffDiagJacobian(unsigned int /*jvar*/)
{
  // if (jvar == _c_var)
  // {
  //  Real dDFDc = (-(14.0 / _int_width) * Dsigma_Deta * _c[_qp] * (1.0 - 3.0 * _c[_qp] + 2.0 * _c[_qp] * _c[_qp])
