...
registerMooseObject("CrowApp", ElectricBCUO);
...
template <> InputParameters validParams<ElectricBCUO>() {
  ...
  params.addParam<MaterialPropertyName>("force_density", "force_density",
...
ElectricBCUO::ElectricBCUO(const InputParameters &parameters)
    : DerivativeMaterialInterface<ElementUserObject>(parameters),
      ...
      _c_var(coupled("c")), _c(coupledValue("c")),
      _grad_c(coupledGradient("c")), _func(getFunction("function")),
      _bc_type(getParam<MooseEnum>("bc_type")) {}

void ElectricBCUO::initialize() { _value = 0.0; }

void ElectricBCUO::execute() {
  Real tol = 0.0001;
...
void ElectricBCUO::finalize() { gatherSum(_value); }

void ElectricBCUO::threadJoin(const UserObject &y) {
  const ElectricBCUO &pps = static_cast<const ElectricBCUO &>(y);
  _value += pps._value;
}

const Real &ElectricBCUO::getBCValues() const { return _value; }
